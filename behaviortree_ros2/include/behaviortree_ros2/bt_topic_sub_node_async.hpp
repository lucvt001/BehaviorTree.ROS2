 ...
 *
 * class SubscriberNode: AsyncRosTopicSubNode<std_msgs::msg::String>
 *
...
class AsyncRosTopicSubNode : public BT::ActionNodeBase
{
public:
  // Type definitions
...
protected:
  ...
  rclcpp::Logger logger()
  {
    ...
    return rclcpp::get_logger("AsyncRosTopicSubNode");
  }
  ...
  double now()
  {
    ...
    return rclcpp::Clock(RCL_ROS_TIME).now().seconds();
  }
...
public:
   ...
   * */
  explicit AsyncRosTopicSubNode(const std::string& instance_name, const BT::NodeConfig& conf,
                           const RosNodeParams& params);

  virtual ~AsyncRosTopicSubNode()
  {
...
//----------------------------------------------------------------
template <class T>
inline AsyncRosTopicSubNode<T>::SubscriberInstance::SubscriberInstance(
    std::shared_ptr<rclcpp::Node> node, const std::string& topic_name)
{
  ...
  // The callback will broadcast to all the instances of AsyncRosTopicSubNode<T>
...
template <class T>
inline AsyncRosTopicSubNode<T>::AsyncRosTopicSubNode(const std::string& instance_name,
...
template <class T>
inline bool AsyncRosTopicSubNode<T>::createSubscriber(const std::string& topic_name)
{
  if(topic_name.empty())
  {
...
template <class T>
inline NodeStatus AsyncRosTopicSubNode<T>::tick()
{
  ...
  {
    std::cout << "Resetting sub instance" << std::endl;
    sub_instance_.reset();
  }

  if(!sub_instance_)
  {
    std::cout << "Recreating subscriber on tick()" << std::endl;
    createSubscriber(topic_name);
  }

  sub_instance_->callback_group_executor.spin_some();
  auto status = onTick(last_msg_);
  if(!latchLastMessage())
  {
    last_msg_.reset();
  }
